...
.auth-stage__grid {
  background-image: linear-gradient(rgba(212, 191, 148, .035) 1px, transparent 1px), linear-gradient(90deg, rgba(212, 191, 148, .035) 1px, transparent 1px);
...
.auth-stage__grid::after {
  background: linear-gradient(90deg, transparent, rgba(212, 191, 148, .35), transparent);
  block-size: 1px;
  content: '';
  inset: 33% 0 auto;
  opacity: .35;
...
.auth-nav__status { align-items: center; color: var(--dim); display: flex; font-family: 'DM Mono', monospace; font-size: .67rem; gap: .55rem; letter-spacing: .08em; text-transform: uppercase; }
...
.auth-layout::before, .auth-layout::after { border-color: rgba(212, 191, 148, .5); border-style: solid; content: ''; block-size: .75rem; inline-size: .75rem; pointer-events: none; position: absolute; z-index: 3; }
...
.auth-rail {
  background: linear-gradient(155deg, rgba(28, 25, 20, .82), rgba(11, 11, 18, .92));
...
.auth-rail::after { background: radial-gradient(circle at 50% 40%, rgba(212, 191, 148, .12), transparent 56%); content: ''; inset: 0; pointer-events: none; position: absolute; }
.auth-rail > * { position: relative; z-index: 1; }
.auth-rail__eyebrow { align-items: center; color: var(--dim); display: flex; font-family: 'DM Mono', monospace; font-size: .66rem; gap: .6rem; letter-spacing: .1em; text-transform: uppercase; }
...
.auth-rail > div > p { color: var(--dim); font-size: .86rem; line-height: 1.7; margin: 0; max-inline-size: 30ch; }
...
.auth-trust-icon { align-items: center; background: rgba(212, 191, 148, .1); border: 1px solid rgba(212, 191, 148, .22); display: inline-flex; flex: 0 0 auto; justify-content: center; block-size: 2.15rem; inline-size: 2.15rem; }
.auth-trust-icon svg { fill: none; stroke: var(--gold); stroke-linecap: round; stroke-linejoin: round; stroke-width: 1.4; block-size: 1.1rem; inline-size: 1.1rem; }
.auth-trust-list strong, .auth-trust-list small { display: block; }
.auth-trust-list strong { color: var(--ivory); font-size: .8rem; font-weight: 600; }
.auth-trust-list small { color: var(--dim); font-size: .7rem; line-height: 1.5; margin-block-start: .2rem; }
.auth-rail__signal { align-items: center; color: var(--dim); display: flex; font-family: 'DM Mono', monospace; font-size: .58rem; gap: .65rem; letter-spacing: .08em; }
.auth-signal-line { background: linear-gradient(90deg, rgba(212, 191, 148, .5), transparent); block-size: 1px; flex: 1; }
.auth-surface { background: rgba(11, 11, 18, .68); padding: clamp(2rem, 5vw, 3.8rem); }
...
.auth-surface__header h1 { color: var(--ivory); font-size: clamp(1.75rem, 3.5vw, 2.35rem); letter-spacing: -.045em; line-height: 1.12; margin: 0; }
.auth-surface__header > div { color: var(--dim); font-size: .85rem; line-height: 1.65; margin-block-start: .75rem; max-inline-size: 45ch; }
.auth-footer { bottom: 1.2rem; color: var(--dim); display: flex; font-family: 'DM Mono', monospace; font-size: .6rem; gap: 2rem; justify-content: space-between; left: clamp(1.5rem, 5vw, 5rem); letter-spacing: .08em; position: absolute; right: clamp(1.5rem, 5vw, 5rem); text-transform: uppercase; }
@media (max-width: 760px) {
  .auth-stage { align-items: flex-start; padding-block: 6rem 5rem; }
  .auth-layout { grid-template-columns: 1fr; }
  .auth-rail { border-block-end: 1px solid var(--line-soft); border-inline-end: 0; min-block-size: auto; padding: 1.4rem; }
